...
#echo "size $m";
my $latency=`sed -n -e '/Flit injection rate = @firvlues[${row}]/,/network throughput/ p' result_100Hz_bsize4_syn_@size_reconfig[${m}].txt | grep "network throughput" | awk '{print \$8}' | head -1`;
#count=$(( ${count} + 1 ));
