...
//#include "router/vcr_credit/vcr_credit.h"
//#include "router/vcr_rt_credit/wormhole_pipeline_power_est.h"
...
int CommonParameter::platform_type = PLATFORM_SYNTHETIC;
...
//	WormholeRouter *wh_router[MAX_DIM][MAX_DIM];
	...
	VCRTRouter<1> *vcrt_router1[MAX_DIM][MAX_DIM];
	VCRTRouter<2> *vcrt_router2[MAX_DIM][MAX_DIM];
	VCRTRouter<4> *vcrt_router4[MAX_DIM][MAX_DIM];
	VCRTRouter<8> *vcrt_router8[MAX_DIM][MAX_DIM];
...
//	AsAPProc *asap_proc[MAX_DIM][MAX_DIM];
	...
	proc_noc_interface *proc_ni[MAX_DIM][MAX_DIM];
...
*/
					...
					string label;
									// input ports
					string direction;
					for (int i = 0; i<N_ROUTER_PORTS; i++){
								switch (i){
									case (WEST):
											direction = "W";
										break;
...
*/
				...
				proc_ni [x][y] = (proc_noc_interface*) noc->tile[x][y]->proc_ni;
				label = "(" + int_to_str(x) + ")(" + int_to_str(y) + ")_"
											+ "ni_r_flit_in";
				sc_trace(wf, noc->tile[x][y]->proc_ni->r_flit_in.read(), label);

				label = "(" + int_to_str(x) + ")(" + int_to_str(y) + ")_"
						+ "ni_r_valid_in";
				sc_trace(wf, noc->tile[x][y]->proc_ni->r_valid_in, label);

				for (int k=0; k<RouterParameter::n_VCs; k++){
					label = "(" + int_to_str(x) + ")(" + int_to_str(y) + ")_"
							+ "ni_r_out_vc_buffer_rd(" + int_to_str(k) + ")";
					sc_trace(wf, noc->tile[x][y]->proc_ni->r_out_vc_buffer_rd[k], label);
				}

				label = "(" + int_to_str(x) + ")(" + int_to_str(y) + ")_"
						+ "ni_r_flit_out";
				sc_trace(wf, noc->tile[x][y]->proc_ni->r_flit_out.read(), label);

				label = "(" + int_to_str(x) + ")(" + int_to_str(y) + ")_"
						+ "ni_r_valid_out";
				sc_trace(wf, noc->tile[x][y]->proc_ni->r_valid_out, label);

				for (int k=0; k<RouterParameter::n_VCs; k++){
					label = "(" + int_to_str(x) + ")(" + int_to_str(y) + ")_"
							+ "ni_r_in_vc_buffer_rd(" + int_to_str(k) + ")";
					sc_trace(wf, noc->tile[x][y]->proc_ni->r_in_vc_buffer_rd[k], label);
				}

				label = "(" + int_to_str(x) + ")(" + int_to_str(y) + ")_"
															+ "ni_p_flit_in";
				sc_trace(wf, noc->tile[x][y]->proc_ni->p_flit_in.read(), label);

				label = "(" + int_to_str(x) + ")(" + int_to_str(y) + ")_"
						+ "ni_p_valid_in";
				sc_trace(wf, noc->tile[x][y]->proc_ni->p_valid_in, label);

				//for (int k=0; k<RouterParameter::n_VCs; k++){
					label = "(" + int_to_str(x) + ")(" + int_to_str(y) + ")_"
							+ "ni_p_out_vc_buffer_rd";
					sc_trace(wf, noc->tile[x][y]->proc_ni->p_out_vc_buffer_rd, label);
				//}

				label = "(" + int_to_str(x) + ")(" + int_to_str(y) + ")_"
						+ "ni_p_flit_out";
				sc_trace(wf, noc->tile[x][y]->proc_ni->p_flit_out.read(), label);

				label = "(" + int_to_str(x) + ")(" + int_to_str(y) + ")_"
						+ "ni_p_valid_out";
				sc_trace(wf, noc->tile[x][y]->proc_ni->p_valid_out, label);

				//for (int k=0; k<RouterParameter::n_VCs; k++){
					label = "(" + int_to_str(x) + ")(" + int_to_str(y) + ")_"
							+ "ni_p_buff_out_full";
					sc_trace(wf, noc->tile[x][y]->proc_ni->p_buff_out_full, label);
				//}

				for (int k=0; k<RouterParameter::n_VCs; k++){
					label = "ni_out_vc_remain";
					label = "(" + int_to_str(x) + ")(" + int_to_str(y) + ")_" + label
							+ "(" + int_to_str(k) + ")";
					sc_trace(wf, noc->tile[x][y]->proc_ni->out_vc_remain[k], label);

					label = "ni_out_vc_remain_reg";
					label = "(" + int_to_str(x) + ")(" + int_to_str(y) + ")_" + label
							+ "(" + int_to_str(k) + ")";
					sc_trace(wf, noc->tile[x][y]->proc_ni->out_vc_remain_reg[k], label);
				}
				label = "ni_in_vc_remain";
				label = "(" + int_to_str(x) + ")(" + int_to_str(y) + ")_" + label;
				sc_trace(wf, noc->tile[x][y]->proc_ni->in_vc_remain[0], label);

				label = "ni_in_vc_remain_reg";
				label = "(" + int_to_str(x) + ")(" + int_to_str(y) + ")_" + label;
				sc_trace(wf, noc->tile[x][y]->proc_ni->in_vc_remain_reg[0], label);

				label = "(" + int_to_str(x) + ")(" + int_to_str(y) + ")_"
						+ "ni_out_buff_empty";
				sc_trace(wf, noc->tile[x][y]->proc_ni->out_buff_empty.read(), label);

				label = "(" + int_to_str(x) + ")(" + int_to_str(y) + ")_"
						+ "ni_in_buff_empty";
				sc_trace(wf, noc->tile[x][y]->proc_ni->in_buff_empty.read(), label);

				label = "(" + int_to_str(x) + ")(" + int_to_str(y) + ")_"
						+ "ni_in_buf_out_buffer_rd";
				sc_trace(wf, noc->tile[x][y]->proc_ni->in_buf_out_buffer_rd.read(), label);

				label = "(" + int_to_str(x) + ")(" + int_to_str(y) + ")_"
						+ "ni_out_buf_out_buffer_rd";
				sc_trace(wf, noc->tile[x][y]->proc_ni->out_buf_out_buffer_rd.read(), label);
...
*/
				...
				// Embedded processor signals
				    ...
				    label = "(" + int_to_str(x) + ")(" + int_to_str(y) + ")_"
																+ "proc_flit_in";
					sc_trace(wf, reconfig_proc[x][y]->flit_in.read(), label);

					label = "(" + int_to_str(x) + ")(" + int_to_str(y) + ")_"
							+ "proc_valid_in";
					sc_trace(wf, reconfig_proc[x][y]->valid_out, label);

					//for (int k=0; k<RouterParameter::n_VCs; k++){
						label = "(" + int_to_str(x) + ")(" + int_to_str(y) + ")_"
								+ "proc_vc_buffer_rd";
						sc_trace(wf, reconfig_proc[x][y]->out_vc_buffer_rd, label);
					//}
					...
					label = "(" + int_to_str(x) + ")(" + int_to_str(y) + ")_"
							+ "proc_valid_out";
					sc_trace(wf, reconfig_proc[x][y]->valid_in, label);

					//for (int k=0; k<RouterParameter::n_VCs; k++){
						label = "(" + int_to_str(x) + ")(" + int_to_str(y) + ")_"
								+ "proc_buff_out_full";
						sc_trace(wf, reconfig_proc[x][y]->in_vc_buffer_rd, label);
					//}
				}
			}	// dim_y loop
		} // dim_x	loop
