	...
	if (argc == 1)
		...
		for (int i=1; i<=argc-1; i++){
			...
			if (!option.compare("-help")){
				...
				else if (!router_type.compare("vct"))
					RouterParameter::router_type = ROUTER_VC_RT;
				else {
			...
			else if (!option.compare("-reconfig_time1")){	// n_hotspots
				ProcessorParameters::proc_reconfig_time_1 = atoi(argv[++i]);
			}
